...
async function fetchQuote(ticker, retries = 3) {
  for (let i = 0; i < retries; i++) {
    try {
      ...
      const low52 = summary.price?.fiftyTwoWeekLow;
      const high52 = summary.price?.fiftyTwoWeekHigh;
      ...
      return {
        ...
        currency: summary.price?.currency || "NOK",
        change1d: summary.price?.regularMarketChangePercent ?? null,
      };
    } catch (err) {
      const is429 = err.message?.includes("Too Many") || err.message?.includes("429");
      if (is429 && i < retries - 1) {
        const wait = (i + 1) * 8000;
        console.log(`⏳ ${ticker}: 429, venter ${wait/1000}s...`);
        await sleep(wait);
      } else {
...
async function setupCommands() {
  ...
  bot.onText(/\/status/, async (msg) => {
    await bot.sendMessage(msg.chat.id, "⏳ Henter data, tar ca. 2 minutter...");
    const state = loadState();
